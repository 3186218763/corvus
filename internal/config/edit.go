...
// separate from the `corvus setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to CORVUS_LANG / locale.
...
// SetPricingCurrency pins the user-global official pricing region independently
// from language. Empty/auto follows the language preference.
func (c *Config) SetPricingCurrency(currency string) error {
	...
	case "USD":
		...
		overridePersisted = true
	default:
		return fmt.Errorf("pricing currency %q: must be auto|CNY|USD", currency)
	}
...
// and legacy JSON receives a disable marker for older Corvus versions.
...
func (c *Config) saveProjectIncrementalResolved(logicalPath, resolvedPath string) error {
	...
	writeProviderAccess := c.UI.ProviderAccess != nil
	...
	if writeProviderAccess {
		body = upsertTOMLSectionKey(body, "ui", "provider_access", "provider_access = "+renderStringArray(c.UI.ProviderAccess))
	}
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# Corvus project configuration.
...
// IsUserConfigPath reports whether path is one of Corvus's current or legacy
