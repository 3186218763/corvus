...
func TestApplyRuntimeAutoPricingCurrency(t *testing.T) {
	tests := []struct {
		...
		{name: "explicit USD wins", runtimeCurrency: "CNY", currency: "USD", wantCurrency: "$", wantOutput: 0.28},
		{name: "explicit CNY wins", runtimeCurrency: "USD", currency: "CNY", wantCurrency: "¥", wantOutput: 2},
		{name: "configured language wins", runtimeCurrency: "USD", language: "zh", wantCurrency: "¥", wantOutput: 2},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := config.Default()
			cfg.UI.Currency = tt.currency
...
func TestBuildLegacyPlanModeReadOnlyCommandsDoesNotEmitWarning(t *testing.T) {
	isolateConfigHome(t)
	dir := robustTempDir(t)
	t.Chdir(dir)
...
func TestAddBuiltinsWithWorkspaceRootKeepsSessionTools(t *testing.T) {
	...
	addBuiltins(reg, nil, nil, []string{robustTempDir(t)}, sandbox.Spec{}, 120*time.Second, builtin.SearchSpec{}, &stderr, robustTempDir(t), netclient.ProxySpec{}, netpolicy.Policy{}, nil, builtin.SessionDataGuard{}, builtin.ManagedConfigPaths{}, nil, nil)
