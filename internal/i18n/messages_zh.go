...
var Chinese = Messages{
	...
	RuntimePolicyHeader:            "运行时策略",
	RuntimePolicyListHint:          "用 /runtime-policy guidance|completion|exposure <值> 设置",
	RuntimePolicyUsage:             "用法：/runtime-policy [guidance inherit|auto|off|light|structured | completion inherit|auto|standard|verified | exposure inherit|auto|eager|deferred]",
	RuntimePolicySwitchUnavailable: "当前会话无法切换运行时策略",
	RuntimePolicySwitchBusy:        "请先完成或取消当前工作再改运行时策略",
	RuntimePolicyAlreadyOnFmt:      "运行时策略 %s 已经是 %s",
	RuntimePolicySwitchingFmt:      "正在将运行时策略 %s 设为 %s…",
	RuntimePolicySwitchedFmt:       "运行时策略 %s 已设为 %s（保留当前对话，但提示词缓存会重新计算）",
	RewindNone:                     "暂无可回滚的内容",
