...
// the codex-exec analogue. It runs exactly one prompt non-interactively and
// exits, streaming structured events (plain text or JSONL) to stdout/stderr
// without requiring a TTY. It reuses the same boot.Build assembly as the
// interactive CLI but drives it through control.Controller.RunTurn.
package main
...
// Exit codes follow the CLI's 1/2 convention: usage errors are 2, run errors
// are 1. A failed turn (TurnDone.Err != nil) is 2; a user/context cancellation
// is 1.
const (
	exitOK      = 0
	exitRunErr  = 1
...
const usageText = `corvus-exec runs a single Corvus prompt headlessly and exits.

Usage:
  corvus-exec [flags] [prompt...]

With no positional arguments the prompt is read from stdin (pipes supported).

Flags:`

// channelSink feeds every emitted event into a buffered channel drained by a
// single writer goroutine, so stream output never interleaves and the agent
// loop never blocks on a slow terminal.
...
func run(args []string, version string) int {
	...
	fs := pflag.NewFlagSet("corvus-exec", pflag.ContinueOnError)
	...
	if *showVersion {
		fmt.Println("corvus-exec", version)
		return exitOK
	}
	...
	opts := boot.Options{
		...
		RequireKey:           true, // headless: fail fast with a clear error when no API key is set
		Sink:                 sink,
		TokenMode:            tokenMode,
...
// parsePermissionMode mirrors internal/cli's mapping for the headless modes
// corvus-exec exposes (plus the CLI's common aliases), without importing cli.
...
// resolveSessionDir is the corvus-exec equivalent of the CLI's
// resolveCLISessionDir: the per-workspace session directory when one is
// configured, otherwise the global one.
...
// resolveSessionQuery is the corvus-exec equivalent of the CLI's resolver: an
// existing file path wins, otherwise the query is matched against session ID,
// filename, custom/topic title, and preview text.
...
// modelForResume picks up a resumed session's own model when --model was not
// given and that model still resolves from configuration (mirrors the CLI).
...
// eventPump renders the event stream: JSONL to stdout in json mode, or
// streamed plain text (answer on stdout, diagnostics on stderr) in text mode.
...
// recordDisplay is the Controller displayRecorder: it captures the full
// user-facing answer so text mode can fall back to it when no Text deltas or
// Message event were seen. Ignored in json mode to keep the JSONL pure.
