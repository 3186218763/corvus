...
func (c *client) Stream(ctx context.Context, req provider.Request) (<-chan provider.Chunk, error) {
	...
	// Track whether the wire body included prompt_cache_key so fail-open only
	// disables when we actually sent the field (not on unrelated 400s).
	keyWasSent := false
	...
	if err != nil {
		// Fail-open: clear reject of prompt_cache_key disables for this endpoint
		// on the next turn. No same-turn auto-retry (avoids surprise double billing).
		if keyWasSent && provider.PromptCacheKeyRejected(err) {
			provider.DisablePromptCacheKey(provider.ProviderFingerprint("responses", c.baseURL))
		}
		return nil, err
	}
	c.authed.Store(true)
