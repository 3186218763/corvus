...
const systemPrompt = "You are corvus, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("CORVUS_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set CORVUS_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("CORVUS_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("CORVUS_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("CORVUS_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
