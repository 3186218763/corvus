...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".corvus/attachments/clipboard-20260601-010203.000000.png": true,
		".corvus/attachments/clipboard-20260601-010203.000000.yml": true,
		".corvus/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".corvus/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".corvus/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".corvus/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".corvus", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".corvus/attachments/config.yml")
	zipRef := filepath.ToSlash(".corvus/attachments/archive.zip")
	pngRef := filepath.ToSlash(".corvus/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: corvus\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: corvus") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".corvus", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".corvus/attachments/shot.png" {
