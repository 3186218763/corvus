...
//
// Project values are read from the workspace file only and are not written into
// the process environment (multi-workspace isolation). The method name is
// historical; resolution is no longer "global-only".
...
func resolveCredentialForRootGlobalFirst(root, key string) CredentialResolution {
	...
	// Prefer the workspace project .env, then fall back to persistent credentials.
	if value, source, ok := projectCredentialValue(root, key); ok {
		res.Set = true
		res.Value = value
		res.Source = source
		res.Source.Label = credentialSourceLabel(res.Source)
...
func projectCredentialValue(root, key string) (string, CredentialSource, bool) {
	...
	if value, ok := envFileValue(path, key); ok && strings.TrimSpace(value) != "" {
		return value, CredentialSource{
			Kind:  CredentialSourceProjectEnv,
			Path:  path,
			Label: "project .env",
		}, true
	}
