...
func TestLoadAndSavePreserveStepFunRegionalBaseURLs(t *testing.T) {
	...
	cfg.UI.ProviderAccess = []string{"stepfun", "stepfun-anthropic"}
...
func TestLoadForEditKeepsLegacyOpenCodeGoKimiK3CatalogMigrationInMemoryUntilSave(t *testing.T) {
	...
	delete(entry.ModelOverrides, "kimi-k3")
	cfg.Providers = append(cfg.Providers, entry)
	cfg.UI.ProviderAccess = []string{"opencode-go"}
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
...
func TestNormalizeUIOfficialProviderAccessCanonicalizesOnlyDeepSeekIDs(t *testing.T) {
	c := Default()
	...
	if len(c.UI.ProviderAccess) != 2 || c.UI.ProviderAccess[0] != "deepseek" || c.UI.ProviderAccess[1] != "mimo-pro" {
		t.Fatalf("provider_access = %+v, want only DeepSeek canonicalized", c.UI.ProviderAccess)
	}
...
func TestNormalizeUIOfficialProviderAccessBackfillsOfficialContextWindow(t *testing.T) {
	c := &Config{
		UI: UIConfig{ProviderAccess: []string{"deepseek-flash", "mimo-api", "mimo-token-plan"}},
...
func TestNormalizeUIOfficialProviderAccessKeepsCustomAlias(t *testing.T) {
	c := &Config{
		UI: UIConfig{ProviderAccess: []string{"deepseek-flash"}},
		Providers: []ProviderEntry{{
			Name:    "deepseek-flash",
			Kind:    "openai",
	...
	if len(c.UI.ProviderAccess) != 1 || c.UI.ProviderAccess[0] != "deepseek-flash" {
		t.Fatalf("provider_access = %+v, want custom alias preserved", c.UI.ProviderAccess)
	}
	if _, ok := c.Provider("deepseek"); ok {
...
func TestNormalizeOfficialDeepSeekModelsRepairsCanonicalProvider(t *testing.T) {
	c := &Config{
		...
		UI:           UIConfig{ProviderAccess: []string{"deepseek"}},
		Providers: []ProviderEntry{{
			Name:      "deepseek",
			Kind:      "openai",
			BaseURL:   "https://api.deepseek.com",
...
func TestNormalizeLegacyMimoCustomProvidersEnsuresReferencedMimoAPI(t *testing.T) {
	...
	c.UI.ProviderAccess = []string{"mimo-api"}
	...
	normalizeUIOfficialProviderAccess(c)
	if got := c.UI.ProviderAccess; len(got) != 1 || got[0] != "mimo-api" {
...
`
	...
	if err := c.SetPricingCurrency("CNY"); err != nil {
		t.Fatalf("SetPricingCurrency CNY: %v", err)
	}
	if flash.Price == nil || flash.Price.Output != 2 || flash.Price.Currency != "¥" {
...
func TestDeepSeekOfficialPricingCurrencyResolution(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "English auto", language: "en", want: "USD"},
		{name: "Chinese auto", language: "zh", want: "CNY"},
		{name: "CLI Chinese fallback", language: "zh", want: "CNY"},
		{name: "explicit USD wins over Chinese", language: "zh", currency: "USD", want: "USD"},
		{name: "explicit CNY wins over English", language: "en", currency: "CNY", want: "CNY"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			c := &Config{Language: tt.language, UI: UIConfig{Currency: tt.currency}}
...
func TestLoadForRootKeepsPricingRegionUserGlobal(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, ".corvus", "config.toml"), []byte("[ui]\ncurrency = \"CNY\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if got := cfg.PricingCurrency(); got != "USD" {
...
func TestNormalizeLegacyMimoProviderCatalogsBackfillsOfficialMimoAPIStub(t *testing.T) {
	c := &Config{
		...
		UI:           UIConfig{ProviderAccess: []string{"mimo-api"}},
		Providers: []ProviderEntry{{
			Name:      "mimo-api",
			Kind:      "openai",
			BaseURL:   "https://api.xiaomimimo.com/v1",
...
func TestNormalizeUIOfficialProviderAccessDoesNotBackfillCustomNamedMimoAPI(t *testing.T) {
	c := &Config{
		UI: UIConfig{ProviderAccess: []string{"mimo-api"}},
		Providers: []ProviderEntry{{
			Name:    "mimo-api",
...
func TestNormalizeLegacyMimoProviderCatalogsBackfillsOfficialMimoTokenPlanStub(t *testing.T) {
	c := &Config{
		UI: UIConfig{ProviderAccess: []string{"mimo-token-plan"}},
		Providers: []ProviderEntry{{
			Name:      "mimo-token-plan",
			Kind:      "openai",
			BaseURL:   "https://token-plan-cn.xiaomimimo.com/v1",
