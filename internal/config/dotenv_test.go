...
// TestLoadDotEnvReadsGlobalCredentials proves `corvus setup`'s target — the
// corvus-owned credentials file under Corvus home — is loaded into the
// process environment from any working directory. Project .env is still not
// imported into process env (provider resolution prefers project separately).
func TestLoadDotEnvReadsGlobalCredentials(t *testing.T) {
	...
	if got := os.Getenv("KEY_SHARED"); got != "global_wins" {
		t.Errorf("global credentials should pin into process env: KEY_SHARED=%q want global_wins", got)
	}
	// Provider credential resolution still prefers project .env over the store.
	got := ResolveCredentialForRootGlobalFirst(cwd, "KEY_SHARED")
	if got.Value != "cwd_wins" || got.Source.Kind != CredentialSourceProjectEnv {
		t.Errorf("provider resolution = %+v, want project .env preferred", got)
	}
}
...
func TestDotEnvFileFilteredPreservesProjectScopeRules(t *testing.T) {
	...
	if _, ok := got["CORVUS_HOME"]; ok {
		t.Fatalf("CORVUS_HOME should be filtered: %+v", got)
	}
	if _, ok := got["HOME"]; ok {
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("CORVUS_STATE_HOME", "")
	os.Unsetenv("CORVUS_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("CORVUS_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${CORVUS_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("CORVUS_STATE_HOME"); got != "" {
...
func TestLoadForRootPrefersProjectProviderEnvOverCredentialsAndInheritedEnv(t *testing.T) {
	project := t.TempDir()
	cfgHome := t.TempDir()
...
`), 0o644); err != nil {
	...
	if got := provider.APIKeySourceLabel(); got != "project .env" {
		t.Fatalf("APIKeySourceLabel = %q, want project .env", got)
	}
	// Project keys stay out of process env; credentials still pin for shared keys.
	if got := os.Getenv(key); got != "from_credentials" {
		t.Fatalf("process env = %q, want credentials pinned (project not imported)", got)
	}
}

func TestResolveCredentialGlobalFirstPrefersProjectWithoutMutatingEnv(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	...
	if got := os.Getenv(key); got != "from_env" {
		t.Fatalf("precondition: existing env should be process value, got %q", got)
	}
	got := ResolveCredentialForRootGlobalFirst(project, key)
	if got.Value != "from_project" || got.Source.Kind != CredentialSourceProjectEnv {
		t.Fatalf("credential = %+v, want project .env over credentials", got)
	}
	foundCredShadow := false
	for _, source := range got.Shadowed {
		if source.Kind == CredentialSourceCredentials {
			foundCredShadow = true
		}
	}
	if !foundCredShadow {
		t.Fatalf("shadowed = %+v, want credentials shadowed by project .env", got.Shadowed)
	}
	if env := os.Getenv(key); env != "from_env" {
		t.Fatalf("project-preferring resolution mutated process env: %q", env)
	}
}

func TestResolveCredentialGlobalFirstFallsBackToCredentials(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	...
	// Empty project value must not win over persistent credentials.
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte(key+"=\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv(key, "")
	os.Unsetenv(key)
	...
	if got.Value != "from_credentials" || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("credential = %+v, want credentials fallback when project key is empty", got)
	}
}
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "Corvus credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want Corvus credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresCorvusStateHome(t *testing.T) {
	home := t.TempDir()
	state := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	want := filepath.Join(home, "corvus-home", ".env")
	...
	if strings.HasPrefix(UserCredentialsPath(), state) {
		t.Fatalf("credentials path must not live under CORVUS_STATE_HOME: %q", UserCredentialsPath())
	}
}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms Corvus-owned global
