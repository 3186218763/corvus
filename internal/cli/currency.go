...
func (m *chatTUI) runCurrencySubcommand(input string) tea.Cmd {
	...
	if len(args) < 2 {
		...
		m.notice(i18n.M.CurrencyHeader + "\n" + describePricingCurrencies(pricingCurrencyDisplay(cfg.PricingCurrency()), cfg.DeepSeekOfficialPricingCurrency()) + "\n" + i18n.M.CurrencyHint)
		return nil
	}
	if len(args) > 2 {
	...
	if err := func() error {
		...
		if err := edit.SetPricingCurrency(mode); err != nil {
			return err
		}
		resolved = edit.DeepSeekOfficialPricingCurrency()
		if mode == "" && edit.ResponseLanguage() == "auto" {
