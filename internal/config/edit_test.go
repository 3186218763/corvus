...
func TestPricingCurrencyNormalizesAndRefreshesOfficialPricing(t *testing.T) {
	c := Default()
	c.Language = "zh"
	if err := c.SetPricingCurrency("usd"); err != nil {
		t.Fatalf("SetPricingCurrency USD: %v", err)
	}
	if got := c.PricingCurrency(); got != "USD" {
		t.Fatalf("pricing currency = %q, want USD", got)
	}
	...
	if err := c.SetPricingCurrency("auto"); err != nil {
		t.Fatalf("SetPricingCurrency auto: %v", err)
	}
	if got := c.PricingCurrency(); got != "" {
		t.Fatalf("auto pricing currency = %q, want empty", got)
	}
	if flash.Price == nil || flash.Price.Output != 2 || flash.Price.Currency != "¥" {
		t.Fatalf("auto Chinese flash price = %+v", flash.Price)
	}
	if err := c.SetPricingCurrency("EUR"); err == nil {
		t.Fatal("SetPricingCurrency accepted unsupported EUR")
	}
}
...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "corvus-plugin-example"}); err != nil {
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "corvus.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	if !strings.Contains(string(userBody), "[ui]") || !strings.Contains(string(userBody), `currency = "CNY"`) {
		t.Fatalf("user config should include TUI preferences:\n%s", userBody)
	}
	...
	if _, err := toml.Decode(string(projectBody), &projectConfig); err != nil {
		t.Fatalf("decode project config: %v", err)
	}
	if got := projectConfig.PricingCurrency(); got != "" {
		t.Fatalf("project config pricing currency = %q, want user-level setting omitted", got)
	}
...
func TestMigrateLegacyMemoryCompilerKeepsMultilineSystemPrompt(t *testing.T) {
	...
	original := `[agent]
system_prompt = """
You are Corvus. Historical config example:
memory_compiler = { enabled = true, verbosity = "compact" }
Keep answers short.
"""
temperature = 0.2
...
func TestSaveToExistingProjectPersistsProviderAccessWithoutReplacingUISection(t *testing.T) {
	...
	cfg.UI.ProviderAccess = []string{"project-relay"}
	if err := cfg.SaveTo(projectPath); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
	body, err := os.ReadFile(projectPath)
	...
	text := string(body)
	for _, want := range []string{"[ui]", `provider_access = ["project-relay"]`, `[permissions]`} {
