...
func newProviderSetupSessionForPath(cfg *config.Config, path string) *providerSetupSession {
	...
	s.accessDeclared = declarations.UIProviderAccessDeclared
...
func (s *providerSetupSession) addProviderAccess(entries []config.ProviderEntry) {
	...
	before := append([]string(nil), s.cfg.UI.ProviderAccess...)
	...
	if !s.accessDeclared && len(s.cfg.UI.ProviderAccess) == 0 {
		if s.projectScoped {
			for _, name := range s.declaredProviders {
				provider, ok := s.cfg.Provider(name)
				if ok && provider.Configured() && len(provider.ModelList()) > 0 {
					s.cfg.UI.ProviderAccess = append(s.cfg.UI.ProviderAccess, name)
				}
			}
		} else {
			config.NormalizeLegacyUIProviderAccess(s.cfg)
		}
		s.accessDeclared = true
		s.operations = append(s.operations, providerSetupOperation{
	...
	seen := make(map[string]bool, len(s.cfg.UI.ProviderAccess)+len(entries))
	for _, name := range s.cfg.UI.ProviderAccess {
...
func (s *providerSetupSession) removeProviderAccess(name string) {
	name = strings.TrimSpace(name)
	if name == "" || len(s.cfg.UI.ProviderAccess) == 0 {
		return
	}
	before := append([]string(nil), s.cfg.UI.ProviderAccess...)
	out := s.cfg.UI.ProviderAccess[:0]
	for _, current := range s.cfg.UI.ProviderAccess {
		if strings.TrimSpace(current) != name {
			out = append(out, current)
		}
	}
	s.cfg.UI.ProviderAccess = out
	s.recordAccessTransition(before)
}

func (s *providerSetupSession) recordAccessTransition(before []string) {
	beforeSet := make(map[string]bool, len(before))
	afterSet := make(map[string]bool, len(s.cfg.UI.ProviderAccess))
	var order []string
	seen := map[string]bool{}
	for _, names := range [][]string{before, s.cfg.UI.ProviderAccess} {
...
func (s *providerSetupSession) replayOperations(cfg *config.Config, accessDeclared *bool, declaredProviders []string) error {
	for _, operation := range s.operations {
		...
		case setupOpMaterializeAccess:
			if *accessDeclared {
				return &providerSetupConflictError{field: "ui.provider_access"}
			}
			...
			if operation.projectScoped {
				for _, name := range declaredProviders {
					...
					if ok && provider.Configured() && len(provider.ModelList()) > 0 {
						cfg.UI.ProviderAccess = append(cfg.UI.ProviderAccess, name)
					}
				}
			} else {
				config.NormalizeLegacyUIProviderAccess(cfg)
			}
			*accessDeclared = true
			if cfg.UI.ProviderAccess == nil {
				cfg.UI.ProviderAccess = []string{}
			}
		case setupOpAccessMembership:
			current := providerSetupAccessContains(cfg.UI.ProviderAccess, operation.accessName)
			if current != operation.beforeBool {
				return &providerSetupConflictError{field: fmt.Sprintf("ui.provider_access[%q]", operation.accessName)}
			}
			if operation.afterBool {
				cfg.UI.ProviderAccess = append(cfg.UI.ProviderAccess, operation.accessName)
			} else {
				out := cfg.UI.ProviderAccess[:0]
				for _, name := range cfg.UI.ProviderAccess {
					if strings.TrimSpace(name) != operation.accessName {
						out = append(out, name)
					}
				}
				cfg.UI.ProviderAccess = out
			}
		default:
...
func commitProviderSetupSession(s *providerSetupSession, configPath string) (bool, error) {
	...
	accessDeclared := declarations.UIProviderAccessDeclared
