...
// the project file is the only config, so isolate CORVUS_HOME: a user-global
...
func TestBuildDoesNotNoticeMissingAPIKeyForNoAuthLoopback(t *testing.T) {
	const keyEnv = "CORVUS_LOCAL_GATEWAY_KEY_FOR_TEST"
	dir := robustTempDir(t)
	t.Chdir(dir)
	t.Setenv(keyEnv, "")
	writeFile(t, dir, filepath.Join(".corvus", "config.toml"), `
...
func TestBuildKeylessDefaultFallsBackToConfiguredProvider(t *testing.T) {
	const keylessEnv = "CORVUS_KEYLESS_DEFAULT_FALLBACK_KEYLESS"
	const configuredEnv = "CORVUS_KEYLESS_DEFAULT_FALLBACK_CONFIGURED"
...
func TestBuildExplicitKeylessModelStillFails(t *testing.T) {
	const keylessEnv = "CORVUS_EXPLICIT_KEYLESS_KEY"
	const configuredEnv = "CORVUS_EXPLICIT_KEYLESS_CONFIGURED"
