...
// LoadCCSwitchMCP reads MCP servers enabled for Corvus from cc-switch and maps
// them to Corvus plugin entries. Newer cc-switch stores servers in SQLite;
// older installs kept them in config.json(.migrated/.bak), so we support both.
//
// CC Switch v16+ stores dedicated enabled_corvus / apps.corvus flags.
// Treat those as authoritative when present, and fall back to Codex only for
// pre-v16 SQLite schemas or legacy JSON entries without Corvus enablement.
...
func loadCCSwitchMCPDB(path string) ([]PluginEntry, error) {
	...
	hasCorvus, err := ccSwitchDBHasCorvusColumn(sqlite, path)
	if err != nil {
		return nil, err
	}
	if hasCorvus {
		query = `SELECT id, name, server_config FROM mcp_servers WHERE enabled_corvus = 1 ORDER BY name, id`
	}
...
func ccSwitchDBHasCorvusColumn(sqlite, path string) (bool, error) {
	const query = `SELECT COUNT(*) FROM pragma_table_info('mcp_servers') WHERE name = 'enabled_corvus'`
	...
	case "1":
		return true, nil
	default:
		return false, fmt.Errorf("cc-switch import: inspect %s: unexpected enabled_corvus column count %q", path, strings.TrimSpace(string(out)))
	}
}
...
func loadCCSwitchLegacyConfig(path string) ([]PluginEntry, error) {
	...
	for _, key := range keys {
		...
		if srv.Apps.Corvus != nil {
			enabled = *srv.Apps.Corvus
		}
		if !enabled {
			continue
		}
