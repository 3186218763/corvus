...
// official DeepSeek defaults. An explicit TUI currency wins; auto follows the
// configured language and finally defaults to USD.
...
// PricingFollowsDetectedLocale reports whether the TUI may supply its terminal
// locale as the official pricing region. Explicit currency or language
// preferences always win.
func (c *Config) PricingFollowsDetectedLocale() bool {
	return c != nil && c.PricingCurrency() == "" && c.ResponseLanguage() == "auto"
}
...
func (c *Config) ApplyRuntimeAutoPricingCurrency(currency string) {
	if !c.PricingFollowsDetectedLocale() {
		return
	}
...
func applyDeepSeekOfficialDefaultPricing(c *Config) {
	applyDeepSeekOfficialDefaultPricingWithOverride(c, c != nil && c.PricingCurrency() != "")
}
