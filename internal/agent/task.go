...
type TaskTool struct {
	...
	// Sticky prompt_cache_key policy inherited from the parent boot entry.
	// SessionCacheID / SubagentCacheID are filled per-run in subagentOptions.
...
type TaskToolOptions struct {
	...
	// Sticky prompt_cache_key policy (from cfg + parent provider entry).
	// Per-run SessionCacheID = ParentSession; SubagentCacheID = transcript Ref.
	PromptCacheKeyMode  string
	PromptCacheKeyValue string
	ProviderKind        string
	ProviderBaseURL     string
...
func (r *ReadOnlyTaskTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	// Ephemeral read_only_task: empty SubagentCacheID; SessionCacheID from ParentSession.
...
func (t *TaskTool) RunProfileSpec(ctx context.Context, spec ProfileExecSpec) (string, error) {
	...
	runSession := func(runCtx context.Context, sink event.Sink, writerAlreadyRegistered bool) (string, error) {
		...
		if spec.ReadOnly {
			return t.runReadOnlySubSession(runCtx, spec.Prompt, subReg, sink, maxSteps, prov, pricing, ctxWin, run.Session, childDepth, recoveryTaskID, usageModelRef, subRef, mutationObserver)
		}
		return t.runSubSession(runCtx, spec.Prompt, subReg, sink, maxSteps, prov, pricing, ctxWin, run.Session, childDepth, recoveryTaskID, usageModelRef, subRef, mutationObserver)
	}
...
func (t *TaskTool) runSubSession(ctx context.Context, prompt string, subReg *tool.Registry, sink event.Sink, maxSteps int, prov provider.Provider, pricing *provider.Pricing, ctxWin int, sess *Session, childDepth int, recoveryTaskID, modelRef, subagentCacheID string, mutationObserver *checkpoint.MutationObserver) (string, error) {
	opts := t.subagentOptions(ctx, maxSteps, pricing, ctxWin, childDepth, recoveryTaskID, subagentCacheID, mutationObserver)
...
func (t *TaskTool) runReadOnlySubSession(ctx context.Context, prompt string, subReg *tool.Registry, sink event.Sink, maxSteps int, prov provider.Provider, pricing *provider.Pricing, ctxWin int, sess *Session, childDepth int, recoveryTaskID, modelRef, subagentCacheID string, mutationObserver *checkpoint.MutationObserver) (string, error) {
	opts := t.subagentOptions(ctx, maxSteps, pricing, ctxWin, childDepth, recoveryTaskID, subagentCacheID, mutationObserver)
	opts.ModelRef = modelRef
	// Capture the pristine task before host framing is prepended: delivery
	// intent classification must judge the task, not the wrapper.
...
//
// SessionCacheID is ParentSession (parent BranchID); empty for headless runs
// without a parent. SubagentCacheID is the transcript Ref (empty ephemeral).
func (t *TaskTool) subagentOptions(ctx context.Context, maxSteps int, pricing *provider.Pricing, ctxWin, childDepth int, recoveryTaskID, subagentCacheID string, mutationObserver *checkpoint.MutationObserver) Options {
	opts := Options{
		MaxSteps:            maxSteps,
		...
		PromptCacheKeyMode:  t.promptCacheKeyMode,
		PromptCacheKeyValue: t.promptCacheKeyValue,
		ProviderKind:        t.providerKind,
		ProviderBaseURL:     t.providerBaseURL,
		SessionCacheID:      ParentSession(ctx),
		SubagentCacheID:     strings.TrimSpace(subagentCacheID),
	}
	return opts
}
