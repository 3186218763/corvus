...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.corvus/config.json.
func legacyConfigPath() string {
	...
	return filepath.Join(home, ".corvus", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.corvus/config.json and returns its enabled
...
func removeMCPJSONApprovalModes(server map[string]json.RawMessage) error {
	...
	// Remove Corvus's retired approval_mode while preserving tool fields owned
