...
// the v0.x ~/.corvus/config.json's mcpServers. Provider api_key_env values
// resolve from Corvus's global .env, not from project .env files.
...
func backfillDeepSeekPro(c *Config) {
	...
	for _, bp := range Default().Providers {
		if bp.Name == "deepseek-pro" {
			...
			if c.PricingCurrency() == "" && flash.persistedOfficialCurrency != "" {
...
func backfillDeepSeekOfficialPrices(c *Config) {
	...
	for i := range c.Providers {
		...
		currency := c.DeepSeekOfficialPricingCurrency()
		if c.PricingCurrency() == "" && p.persistedOfficialCurrency != "" {
...
func mergeTOMLProviderAccess(paths []string) ([]string, bool, error) {
	...
	for _, path := range paths {
		...
		if !saw {
			...
			// means the user intentionally removed every provider.
			merged = []string{}
		}
		saw = true
		for _, name := range f.UI.ProviderAccess {
...
func InspectConfigFileDeclarations(path string) (ConfigFileDeclarations, error) {
	...
	declarations.UIProviderAccessDeclared = meta.IsDefined("ui", "provider_access")
	return declarations, nil
}

// UIProviderAccessDeclared reports whether path explicitly declares
// ui.provider_access. It distinguishes omission from an intentional [].
func UIProviderAccessDeclared(path string) (bool, error) {
	declarations, err := InspectConfigFileDeclarations(path)
	return declarations.UIProviderAccessDeclared, err
}

// LoadForEdit returns a config to seed the `corvus setup` wizard when reconfiguring:
...
// of resetting to defaults. Corvus's global .env is loaded so api_key_env
...
func legacyMimoConfigRefs(c *Config) []string {
	...
	refs = append(refs, c.UI.ProviderAccess...)
	return refs
}
...
func normalizeUIOfficialProviderAccess(c *Config) {
	if c == nil || len(c.UI.ProviderAccess) == 0 {
		return
	}
	seen := uiProviderAccessMap(nil)
	next := make([]string, 0, len(c.UI.ProviderAccess))
	for _, name := range c.UI.ProviderAccess {
		name = uiProviderAccessNameForConfig(c, name)
		if name == "" || seen[name] {
			continue
		}
		seen[name] = true
	...
	c.UI.ProviderAccess = next
	if seen["deepseek"] {
		ensureDeepSeekOfficialProvider(c)
	}
	normalizeLegacyMimoProviderCatalogs(c)
	retargetOfficialProviderRefs(c, seen)
}

// NormalizeLegacyUIProviderAccess seeds the TUI provider-access list for
// configs written before setup tracked explicit provider access. Callers
...
func NormalizeLegacyUIProviderAccess(c *Config) {
	if c == nil || len(c.UI.ProviderAccess) > 0 {
		return
	}
	seen := uiProviderAccessMap(nil)
...
func uiProviderAccessNameForConfig(c *Config, name string) string {
	name = strings.TrimSpace(name)
	if name == "" {
		return ""
	}
	canonical := canonicalOfficialProviderName(name)
	if canonical == name {
		return name
	}
	if c == nil {
...
func ensureDeepSeekOfficialProvider(c *Config) {
	...
	if old, ok := c.Provider("deepseek-flash"); ok {
		...
		currency := c.DeepSeekOfficialPricingCurrency()
		if c.PricingCurrency() == "" && old.persistedOfficialCurrency != "" {
...
func retargetOfficialProviderRefs(c *Config, access map[string]bool) {
	c.DefaultModel = retargetOfficialProviderRef(c.DefaultModel, access)
	c.Agent.PlannerModel = retargetOfficialProviderRef(c.Agent.PlannerModel, access)
	c.Agent.SubagentModel = retargetOfficialProviderRef(c.Agent.SubagentModel, access)
	for skill, ref := range c.Agent.SubagentModels {
		c.Agent.SubagentModels[skill] = retargetOfficialProviderRef(ref, access)
	}
}

func retargetOfficialProviderRef(ref string, access map[string]bool) string {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return ""
	}
