...
type Agent struct {
	...
	// Sticky OpenAI-family prompt_cache_key routing. Copied from Options at New;
	// sessionCacheID may be refreshed via SetSessionCacheID after resume/new session.
...
// prompt_cache_key resolution. Call after resume/new session when the agent is
// kept and only the session path changes.
func (a *Agent) SetSessionCacheID(id string) {
	if a == nil {
		return
	}
	a.sessionCacheID = strings.TrimSpace(id)
}

// promptCacheKeyDisableNotices tracks fingerprints for which we already emitted
// the fail-open Notice this process, so users are not spammed every stream.
var promptCacheKeyDisableNotices sync.Map // fingerprint string → struct{}

const promptCacheKeyDisabledNoticeText = `Disabled prompt_cache_key for this provider endpoint after the API rejected the field. Set [agent].prompt_cache_key = "off" to silence, or upgrade/fix the gateway.`

// maybeNoticePromptCacheKeyDisabled emits one LevelWarn Notice per fingerprint
// per process when sticky keys are fail-open disabled.
func (a *Agent) maybeNoticePromptCacheKeyDisabled(fp string, sink event.Sink) {
	fp = strings.TrimSpace(fp)
	if fp == "" {
		return
	}
	if _, loaded := promptCacheKeyDisableNotices.LoadOrStore(fp, struct{}{}); loaded {
		return
	}
	if sink == nil {
		sink = a.sink
	...
	sink.Emit(event.Event{
		...
		Text:  promptCacheKeyDisabledNoticeText,
	})
}
...
type Options struct {
	...
	// PromptCacheKeyMode selects sticky prompt-cache key policy for OpenAI-family
	// providers: auto|on|off|custom. Empty is treated as auto at resolve time.
	PromptCacheKeyMode string
	// PromptCacheKeyValue is the raw sticky key used only when mode is custom.
	PromptCacheKeyValue string
	// ProviderKind is the provider kind string (openai, responses, anthropic, …)
	// used by ResolvePromptCacheKey.
	ProviderKind string
	// ProviderBaseURL is the provider endpoint base URL for DeepSeek-shape and
	// fail-open fingerprint checks.
	ProviderBaseURL string
	// SessionCacheID is the BranchID-derived session sticky id; empty omits the key.
	// Updated when the session path changes (see SetSessionCacheID).
	SessionCacheID string
	// SubagentCacheID is SubagentMeta.Ref for nested agents; empty for the main agent.
	SubagentCacheID string
}
...
func New(prov provider.Provider, tools *tool.Registry, session *Session, opts Options, sink event.Sink) *Agent {
	...
	a := &Agent{
		...
		promptCacheKeyMode:        opts.PromptCacheKeyMode,
		promptCacheKeyValue:       opts.PromptCacheKeyValue,
		providerKind:              opts.ProviderKind,
		providerBaseURL:           opts.ProviderBaseURL,
		sessionCacheID:            strings.TrimSpace(opts.SessionCacheID),
		subagentCacheID:           strings.TrimSpace(opts.SubagentCacheID),
	}
...
func (a *Agent) stream(ctx context.Context, turn int, sink event.Sink) (string, string, string, []provider.ToolCall, *provider.Usage, bool, bool, []provider.ToolCall, error) {
	...
	// Fail-open UX: when Resolve omits because this endpoint rejected the field
	// earlier, surface one Notice per fingerprint per process.
	if key == "" {
		fp := provider.ProviderFingerprint(a.providerKind, a.providerBaseURL)
		if provider.IsPromptCacheKeyDisabled(fp) {
			a.maybeNoticePromptCacheKeyDisabled(fp, sink)
		}
	}
