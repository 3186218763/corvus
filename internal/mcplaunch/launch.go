...
// Scope remains in the JSON solely so previous Corvus versions can read new
...
// ForWorkspace returns the process-shared manager for one Corvus home and
// workspace so sibling tabs observe one launch authorization state.
func ForWorkspace(corvusHome, workspace string) *Manager {
	path := StatePath(corvusHome)
