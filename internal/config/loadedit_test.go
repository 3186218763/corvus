...
func TestLoadForEditDoesNotBindEditedConfigRootCredentials(t *testing.T) {
	project := t.TempDir()
	launch := t.TempDir()
	home := t.TempDir()
...
`
	...
	// Edit load uses cwd for credential fallback, not the edited file's directory.
	if provider.Configured() {
		t.Fatalf("provider should not resolve api_key_env from edited config's sibling .env while cwd differs")
	}
	// Runtime resolution for that project root does prefer its project .env.
	got := ResolveCredentialForRootGlobalFirst(project, "PROJECT_ONLY_KEY")
	if !got.Set || got.Value != "from-project" || got.Source.Kind != CredentialSourceProjectEnv {
		t.Fatalf("credential = %+v, want project .env for workspace root", got)
	}
}
