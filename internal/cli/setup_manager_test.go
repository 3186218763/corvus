...
func setupTestConfig() *config.Config {
	...
	cfg.UI.ProviderAccess = []string{"desktop-provider", "cli-provider"}
	return cfg
}
...
func TestProviderSetupSessionAddPreservesExistingProvidersAndSettings(t *testing.T) {
	...
	if got := cfg.UI.ProviderAccess; !containsString(got, "desktop-provider") || !containsString(got, "cli-provider") || !containsString(got, "grok-relay") {
...
func TestProviderSetupSessionRemovalIsExplicitAndRepairsDefault(t *testing.T) {
	...
	if containsString(cfg.UI.ProviderAccess, "desktop-provider") || !containsString(cfg.UI.ProviderAccess, "cli-provider") {
		t.Fatalf("desktop provider access was not cleaned safely: %v", cfg.UI.ProviderAccess)
	}
}
...
func TestProviderSetupSessionAddAccessRespectsExplicitEmptyList(t *testing.T) {
	...
	s.addProviderAccess([]config.ProviderEntry{added})
	if got := cfg.UI.ProviderAccess; len(got) != 1 || got[0] != "grok-relay" {
...
func TestProviderSetupSessionAddAccessSeedsUndeclaredLegacyProviders(t *testing.T) {
	...
	if got := cfg.UI.ProviderAccess; !containsString(got, "cli-provider") || !containsString(got, "grok-relay") {
		t.Fatalf("undeclared legacy access should preserve configured siblings and add the new provider: %v", got)
	}
	count := 0
	for _, name := range cfg.UI.ProviderAccess {
		if name == "grok-relay" {
			count++
		}
	}
	if count != 1 {
		t.Fatalf("added provider access should be deduplicated: %v", cfg.UI.ProviderAccess)
	}
}
...
`), 0o600); err != nil {
	...
	if got := reloaded.UI.ProviderAccess; !containsString(got, "project-relay") || !containsString(got, "grok-relay") {
...
func TestNewProviderSetupSessionDetectsExplicitProviderAccess(t *testing.T) {
	...
	if !s.accessDeclared {
		t.Fatal("explicit empty ui.provider_access was treated as undeclared")
	}
}
...
func TestProviderSetupSessionPersistsEmptyAccessAfterLastRemoval(t *testing.T) {
	isolateUserConfig(t)
	cfg := setupTestConfig()
	cfg.UI.ProviderAccess = []string{"desktop-provider"}
	s := newProviderSetupSession(cfg)
	s.accessDeclared = true
	if err := s.remove("desktop-provider"); err != nil {
		t.Fatal(err)
	}
	if cfg.UI.ProviderAccess == nil || len(cfg.UI.ProviderAccess) != 0 {
		t.Fatalf("last removal should retain an explicit empty access list: %#v", cfg.UI.ProviderAccess)
	}
	path := config.UserConfigPath()
	if err := cfg.SaveTo(path); err != nil {
		t.Fatal(err)
...
func TestProviderSetupOperationReplayMergesConcurrentAccessAddition(t *testing.T) {
	...
	external.UI.ProviderAccess = append(external.UI.ProviderAccess, externalProvider.Name)
	if err := external.SaveTo(path); err != nil {
		t.Fatal(err)
	}
	...
	for _, name := range []string{setupProvider.Name, externalProvider.Name} {
		if _, ok := got.Provider(name); !ok || !containsString(got.UI.ProviderAccess, name) {
			t.Fatalf("provider/access %q missing after replay: providers=%v access=%v", name, got.Providers, got.UI.ProviderAccess)
		}
	}
}
...
func TestProviderSetupOperationReplayRejectsConcurrentAccessDeclaration(t *testing.T) {
	...
	external := config.LoadForEdit(path)
	external.UI.ProviderAccess = []string{}
	if err := external.SaveTo(path); err != nil {
		t.Fatal(err)
	}
	...
	_, err := commitProviderSetupSession(s, path)
	var conflict *providerSetupConflictError
	if !errors.As(err, &conflict) || conflict.field != "ui.provider_access" {
		t.Fatalf("commit conflict = %v, want provider_access declaration conflict", err)
	}
	declared, err := config.UIProviderAccessDeclared(path)
	if err != nil {
		t.Fatal(err)
	}
	got := config.LoadForEdit(path)
	if !declared || got.UI.ProviderAccess == nil || len(got.UI.ProviderAccess) != 0 {
		t.Fatalf("external explicit empty access was overwritten: declared=%v access=%#v", declared, got.UI.ProviderAccess)
	}
}
...
`
	...
	for _, name := range []string{"initial-project", "external-project", "setup-provider"} {
		if _, ok := got.Provider(name); !ok || !containsString(got.UI.ProviderAccess, name) {
			t.Fatalf("latest project provider/access %q missing: providers=%v access=%v", name, got.Providers, got.UI.ProviderAccess)
		}
	}
	for _, forbidden := range []string{"deepseek", "deepseek-flash", "deepseek-pro"} {
		if containsString(got.UI.ProviderAccess, forbidden) {
			t.Fatalf("materialized project access unexpectedly enabled %q: %v", forbidden, got.UI.ProviderAccess)
		}
	}
}
...
func TestLocalSetupPersistsWorkspaceProviderAccess(t *testing.T) {
	...
	text := string(body)
	if !strings.Contains(text, "[ui]") || !strings.Contains(text, `provider_access = ["grok-relay"]`) {
		t.Fatalf("local setup omitted workspace UI access:\n%s", text)
	}
	if strings.Contains(text, "\ntheme_style =") || strings.Contains(text, "\ndefault_tool_approval_mode =") {
		t.Fatalf("local setup leaked user-global UI preferences:\n%s", text)
	}
}
