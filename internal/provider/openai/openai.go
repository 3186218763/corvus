...
func reservedExtraBodyField(name string) bool {
	...
	case "model", "messages", "tools", "stream", "stream_options", "temperature", "max_tokens", "max_completion_tokens", "max_output_tokens", "reasoning_effort", "thinking", "prompt_cache_key":
		return true
	default:
		return false
	}
...
func (c *client) openStream(ctx context.Context, targetURL string, wireReq chatRequest, tools []provider.ToolSchema) (<-chan provider.Chunk, error) {
	...
	if err != nil {
		// Fail-open: if we sent prompt_cache_key and the gateway clearly rejected
		// the field, disable for this fingerprint so the next turn omits it.
		// No same-turn auto-retry (avoids surprise double billing).
		if keyWasSent := strings.TrimSpace(wireReq.PromptCacheKey) != ""; keyWasSent && provider.PromptCacheKeyRejected(err) {
...
func normaliseUsage(u *wireUsage) *provider.Usage {
	...
	if anthropicPrompt {
		...
		// writes, while Corvus PromptTokens represents the complete input.
	...
	if miss == 0 {
		...
		case anthropicPrompt:
			// Cache writes are still uncached input for Corvus pricing and
...
func (r chatRequest) MarshalJSON() ([]byte, error) {
	...
	// Canonical Request.PromptCacheKey wins; ExtraBody cannot inject/override.
	if r.PromptCacheKey != "" {
