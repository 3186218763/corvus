...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing CORVUS.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "CORVUS.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "CORVUS.md" {
		t.Errorf("should append to the existing CORVUS.md, got %s", got)
	}
